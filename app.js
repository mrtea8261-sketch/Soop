...
const el = {
  ...
  guessButtons:document.getElementById("guessButtons"), message:document.getElementById("message"), pathBox:document.getElementById("pathBox"), newGameBtn:document.getElementById("newGameBtn"), leaderboard:document.getElementById("leaderboard"), clearScoresBtn:document.getElementById("clearScoresBtn"), nameDialog:document.getElementById("nameDialog"), nameForm:document.getElementById("nameForm"), playerName:document.getElementById("playerName")
};
...
function cleanGraph(raw){
  ...
  const by = Object.fromEntries(nodes.map(n=>[n.id,n])); const ids = new Set(nodes.map(n=>n.id));
  const edges=[]; const seen=new Set();
  for(const e of raw.edges||[]){ const a=e.a||e.from, b=e.b||e.to; if(!a||!b||a===b||!ids.has(a)||!ids.has(b)) continue; const key=[a,b].sort().join("__"); if(seen.has(key)) continue; seen.add(key); edges.push({a,b,aNick:by[a]?.nick,bNick:by[b]?.nick}); }
  const adjacency=buildAdj(edges);
  const connected = nodes.filter(n => adjacency[n.id]?.length);
  const connectedIds = new Set(connected.map(n=>n.id));
  const finalEdges = edges.filter(e=>connectedIds.has(e.a)&&connectedIds.has(e.b));
  return { nodes:connected, nodesById:Object.fromEntries(connected.map(n=>[n.id,n])), edges:finalEdges, adjacency:buildAdj(finalEdges) };
}
...
async function maybeSaveHighScore(score){ if(score<=0) return; const scores=await getScores(); if(!(scores.length<5||score>scores[scores.length-1].score)) return; el.playerName.value=""; el.nameDialog.showModal(); el.nameForm.onsubmit=async ev=>{ ev.preventDefault(); await addScore(el.playerName.value.trim()||"익명",score); el.nameDialog.close(); await renderLeaderboard(); updateScoreDisplay(); }; }
